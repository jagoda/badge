...
exports.register = function (plugin, options, done) {
	...
	function checkOrganization (organization, token, result) {
		var options = {
			headers : {
				authorization : "Basic " + (new Buffer(token + ":x-oauth-basic")).toString("base64")
			}
		};
			...
			authenticate : function (request, reply) {
				...
				.then(function (result) {
					if (config.organization) {
						return checkOrganization(config.organization, credentials.token, result);
					}

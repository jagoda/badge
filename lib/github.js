...
exports.register = function (plugin, options, done) {
	...
	function basicLogin (options, result) {
		...
		.spread(function (response, payload) {
			...
			if (response.statusCode === 200) {
				...
				throw Hapi.error.unauthorized("login failed", BASIC_SCHEME);
			}
	...
	function checkOrganization (options) {
		...
		return Q.ninvoke(Nipple, "get", orgUrl, request)
		.spread(function (response) {
			...
			if (response.statusCode === 204) {
				...
				throw Hapi.error.unauthorized("not authorized", options.scheme);
			}
	...
	function getToken (config, options, result) {
		...
		.spread(function (response, payload) {
			...
			if (_.indexOf([ 200, 201 ], response.statusCode) >= 0) {
				...
				throw Hapi.error.unauthorized("token request failed", TOKEN_SCHEME);
			}
	...
	function tokenLogin (tokenUrl, options, result) {
		...
		.spread(function (response, payload) {
			...
			if (response.statusCode === 200) {
				...
				throw Hapi.error.unauthorized("invalid token", TOKEN_SCHEME);
			}
		...
		return {
			authenticate : function (request, reply) {
				...
				if (!result.credentials.username) {
					...
					reply(Hapi.error.unauthorized("forbidden", BASIC_SCHEME), result);
					return;
				}
		...
		return {
			authenticate : function (request, reply) {
				...
				if (!credentials.token) {
					...
					reply(Hapi.error.unauthorized("forbidden", TOKEN_SCHEME), result);
					return;
				}
